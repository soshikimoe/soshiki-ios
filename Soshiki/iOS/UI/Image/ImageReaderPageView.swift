...
class ImageReaderPageView: UIView {
    let progressView = CircularProgressView(frame: .init(x: 0, y: 0, width: 40, height: 40))
    let imageView = UIImageView(frame: UIScreen.main.bounds)
    let zoomableView = ZoomableView(frame: UIScreen.main.bounds)
    let reloadButton = UIButton(frame: .init(x: 0, y: 0, width: 40, height: 40))

    var image: String?
    var source: any ImageSource

    init(_ image: String? = nil, source: any ImageSource) {
        self.image = image
        self.source = source
