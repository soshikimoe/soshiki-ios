...
class VideoPlayerViewController: BaseViewController {
    ...
    var previousEpisode: VideoSourceEpisode? {
        episodes[safe: episodeIndex + 1]
    }
    var currentEpisode: VideoSourceEpisode? {
        episodes[safe: episodeIndex]
    }
...
extension VideoPlayerViewController {
    ...
    func setUrl(to url: URL, persistTimestamp: Bool = false) async {
        let cachedTime = self.playerLayer.player?.currentTime().seconds
            ...
            children: ([3, 2, 1.5, 1.25, 1, 0.5, 0.25] as [Double]).map({ rate in
                UIAction(
                    ...
                    handler: { [weak self] _ in
                        ...
                        if let player = self?.playerLayer.player, player.rate != 0 { // Only change rate when playing
...
extension VideoPlayerViewController {
    ...
    @objc func airplayButtonPressed(_ sender: UIButton? = nil) {
        ...
        if let button = self._airplayButton.subviews.first(where: { $0 is UIButton }) as? UIButton {
